...
if($_POST["task_id"])
    {
      $taskid = $_POST["task_id"];
         ...
         if($statement->execute($data))
         {
          ...
          $client_id = 'phpMQTT-iot-dashboard-pub-off'; // make sure this is unique for connecting to sever - you could use uniqid()
          
          
          $mqtt = new Bluerhinos\phpMQTT($server, $port, $client_id);
          if ($mqtt->connect(true, NULL, $username, $password)) {
            $mqtt->publish('offTopic', $taskid, 0, false);
            $mqtt->close();
          } else {
              echo "Time out!\n";
          }
